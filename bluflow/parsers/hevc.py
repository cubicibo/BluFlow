...
class HEVCIndexer(Indexer):
    ...
    def get_pts_dts_of_access_unit(
        ...
        From
        ----
        access_units : Iterable[AccessUnit]
        ...
        pic_duration = MPEGClock.PTS * Fraction(
            au.sequence_parameter_set['vui']['vui_num_units_in_tick'],
            au.sequence_parameter_set['vui']['vui_time_scale'])

        picture_timing = au.sei[SEI.PictureTiming]
        pts = first_pts
        dts = pts - picture_timing['pic_dpb_output_delay'] * pic_duration
        
        last_buffering_sei_ts = first_pts - pic_duration
        ...
        if (dts_shift := (last_buffering_sei_ts - dts)) > 0:
            ...
            print(f"b-pyramid detected: shift DTS by {dts_shift/pic_duration} frames.")
        ...
        while True:
            ...
            picture_timing = au.sei[SEI.PictureTiming]
            dts = last_buffering_sei_ts + (picture_timing['au_cpb_removal_delay_minus1'] + 1) * pic_duration
            pts = dts + (picture_timing['pic_dpb_output_delay'] * pic_duration)
...
def VBV_Verifier(nals: list[AccessUnit]) -> list[int]:
    """
    Given a decoded bitstream (list of access units), with HRD information
    return a list of VBV sample points on a VUI timebase grid
    """
    
    assert SEI.BufferingPeriod in nals[0].sei, "First picture requires a buffering period"
    hrd_bp = nals[0].sei[SEI.BufferingPeriod]['NalHrdBp']
    ...
    hrd_parameters = nals[0].sequence_parameter_set['vui']['hrd_parameters']
    baseCpbHrdParam =  hrd_parameters['sub_layer_hrd_parameters'][0]
    assert baseCpbHrdParam['fixed_pic_rate_general_flag']
    ...
    cbr_flag = baseCpbHrdParam['cbr_flag']
    
    init_fill = layer_cpb_init['nal_initial_cpb_removal_delay'] + layer_cpb_init['nal_initial_cpb_removal_offset']
    
    pic_duration = MPEGClock.PTS * Fraction(au.sequence_parameter_set['vui']['vui_num_units_in_tick'],
                                            au.sequence_parameter_set['vui']['vui_time_scale'])
    
    for m, nal in enumerate(nals):
        picture_timing = nal.sei[SEI.PictureTiming]
        has_buffering_period = SEI.BufferingPeriod in nal
        if has_buffering_period:
            last_buffering_period = au.sei[SEI.BufferingPeriod]

        if cbr_flag:
            initArrivalTime = auFinalArrivalTime  
        else:
            if m > 0:
                
            else:
                AuNominalRemovalTime = layer_cpb_init['nal_initial_cpb_removal_delay']
            tmpNominalRemovalTime = picture_timing[au_cpb_removal_delay_minus1] +  1
            
            
            initArrivalTime = max(auFinalArrivalTime, initArrivalEarliestTime)
        auFinalArrivalTime = 
